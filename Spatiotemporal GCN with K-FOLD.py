...
def k_fold_cross_validation(data, k, unified_model, batch_size, l2_lambda):
    kf = GroupKFold(n_splits=k)  # Use GroupKFold to ensure no data leakage by users
    fold = 1  # Starting fold
    total_rmse = 0
    total_mae = 0

    # Perform K-fold cross-validation, splitting by userId (so that traind and test data are completely different)
    for train_index, test_index in kf.split(data, groups=data['userId']):
        print(f"\n--- Fold {fold} ---")
        
        # Splitting the data into train and test based on indices
        ...
        # Use the Adam Stochastic optimizer for the main unified model with learning rate 0.001
        optimizer = optim.Adam(unified_model.parameters(), lr=1e-3)
    ...
    print(f"\nAverage RMSE across {k} folds: {avg_rmse}")
...
def Model_k_fold(k):
    ...
    k_fold_cross_validation(data, k, unified_model, BATCH_SIZE, L2_LAMBDA)
